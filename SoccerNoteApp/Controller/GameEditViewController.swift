...
class GameEditViewController: UIViewController, UITextFieldDelegate, UINavigationBarDelegate {
    ...
    @IBAction func didTapEditButton(_ sender: UIButton) {
        GameDataUpdateModel.updateGameData(updGameDate: gameEditDatePicker.date,
                                           updTeam: teamEditTextField.text ?? "",
                                           updMyScore: myScoreEditTextField.text ?? "",
                                           updOpponentScore: opponentScoreEditTextField.text ?? "",
                                           updFirstHalf: firstHalfEditTextView.text,
                                           updSecondHalf: secondHalfEditTextView.text,
                                           updConclusion: conclusionEditTextView.text)
        self.dismiss(animated: true, completion: nil)
    }
    
    func gameManagementData() {
